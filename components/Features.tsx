      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-3 gap-16">
          <div className="lg:col-span-1">
            ...
            <h3 className="text-4xl md:text-5xl font-black text-white mb-8 tracking-tighter">Engenharia de <span className="text-[#FFB35B]">Alta Performance</span></h3>
          ...
          <div className="lg:col-span-2 grid sm:grid-cols-2 gap-8">
            {TECH_SPECS.map((tech, idx) => (
              <motion.div 
                ...
                className="glass-card p-10 rounded-[32px] border-l-4 border-l-[#007BFF]"
              >
                ...
                <h4 className="text-2xl font-black text-white mb-4 tracking-tight">{tech.title}</h4>
                <p className="text-slate-400 text-lg leading-relaxed font-medium">{tech.description}</p>
              </motion.div>
            ))}
            <motion.div 
              ...
              viewport={{ once: true }}
              className="glass-card p-10 rounded-[32px] flex flex-col justify-center items-center text-center border border-dashed border-white/20 group hover:border-[#FFB35B]/50 transition-all"
            >
               <h4 className="text-xl font-black text-white mb-4 uppercase tracking-tighter">Roadmap 2025</h4>
               <p className="text-slate-400 mb-8 font-medium">Integração nativa com hidrômetros NB-IoT e LoRaWAN.</p>
               <button className="text-[#FFB35B] font-black hover:translate-x-2 transition-transform uppercase tracking-widest text-xs">Acompanhar Evolução →</button>
            </motion.div>
          </div>
        </div>
      </div>
